...
{
    uint32_t Bp[NCOEFFS_B][NSHARES], Cp[NCOEFFS_C][NSHARES];
    uint32_t result[NSHARES];
    // uint32_t U[NSHARES][NSHARES-1];
    ...
    for (size_t i = 0; i < NCOEFFS_B; i++)
    {
        Bp[i][0] = Bp[i][0] - (public_B[i] << (COMPRESSFROM_B - COMPRESSTO_B)) ;
    }

    for (size_t i = 0; i < NCOEFFS_C; i++)
    {      
        Cp[i][0] = Cp[i][0] - (public_C[i] << (COMPRESSFROM_C - COMPRESSTO_C));
    }
    ...
    // run A2C over all coefficients
    const uint32_t template_B = (1 << COMPRESSFROM_B) - (1 << (COMPRESSFROM_B - COMPRESSTO_B));
    for (size_t i = 0; i < NCOEFFS_B; i += 1)
    {
        #if defined(NORANDOMSAMPLING)
            rng_idx = 0; // to avoid overflow of randomness register
        #endif
        A2C(result, Bp[i], template_B); 
    }

    const uint32_t template_C = (1 << COMPRESSFROM_C) - (1 << (COMPRESSFROM_C - COMPRESSTO_C));
    for (size_t i = 0; i < NCOEFFS_C; i += 1)
    {
        #if defined(NORANDOMSAMPLING)
            rng_idx = 0;
        #endif
        A2C(result, Cp[i], template_C); 
    }

    // unmask the final result
    uint32_t unmasked_result = 0;
    for (size_t sh = 0; sh < NSHARES; sh++)
    {
        unmasked_result ^= result[sh];
    } 
