...
static int test_MaskedComparison(void)
{
    ...
    for (size_t i = 0; i < NTESTS; i++)
    {
        ...
        #endif
        #ifdef ROTATE
        hal_send_str("===== Start (unmodified ct) ROTATE ====");
        PROFILE_TOP_START();
        result = MaskedComparison_Rotate(B, C, public_B, public_C);
        #endif
        #ifdef SIMPLE
        ...
        PROFILE_TOP_START();
        result = MaskedComparison_Hybrid(B, C, public_B, public_C);
        #endif
        #ifdef HYBRIDSIMPLE
        hal_send_str("===== Start (unmodified ct) Hybrid ====");
        PROFILE_TOP_START();
        ...
        #endif
        #ifdef ROTATE
        hal_send_str("===== Start (modified ct) ROTATE ====");
        PROFILE_TOP_START();
        result = MaskedComparison_Rotate(B, C, public_B, public_C);
        #endif
        #ifdef SIMPLE
