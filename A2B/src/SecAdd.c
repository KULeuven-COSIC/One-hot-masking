...
{
	uint32_t xXORy[nshares], xANDy[nshares];
	uint32_t c_bit[nshares], xANDy_bit[nshares], cANDxXORy_bit[nshares], xXORy_bit[nshares];
	...
	SecAND32(nshares, xANDy, x, y);
	// sum
	SecXOR32(nshares, xXORy, x, y);

	// carry out
	for (size_t j = 0; j < BITS - 1; j++) //* (nbits=64) - 1 because we don't need final carry
	{
		for (size_t i = 0; i < nshares; i++)
		{
			xANDy_bit[i] = (xANDy[i] >> j) & 0x1;
		...
		{
			c_bit[i] = xANDy_bit[i] ^ cANDxXORy_bit[i];
			z[i] |= (uint32_t)(c_bit[i] & 0x1) << (j + 1); 
		}
	}

	SecXOR32(nshares, z, xXORy, z);
}
